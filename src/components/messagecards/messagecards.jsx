...
import { arrayUnion, collection, doc, getDoc, getDocs, onSnapshot, orderBy, query, updateDoc } from 'firebase/firestore';
...
const Threads = ({ onLike, onAddComment }) => {
 ...
 const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const querySnapshot = await getDocs(collection(db, 'threads'), orderBy('createdAt', 'desc'));
    ...
    const unsubscribe = onSnapshot(query(collection(db, 'threads'), orderBy('createdAt', 'desc')), (snapshot) => {
    ...
    <div>
       {threads.map((thread, index) => (
        <div key={index} className='threads-container'>
          ...
          <UserAvatar username={userRole === 'admin' ? thread.trueusername : thread.username} />
          <div style={{ display: 'flex', width: '500px', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
           <h3 className='username' style={{ margin: '5px',fontSize:"18px"}}>     
                {userRole === 'admin' ? thread.trueusername : thread.username}
</h3>
           ...
           <ThreadOptions thread={thread} threadId={thread.id} userRole={userRole} currentUsername={studentNum} username={thread.trueusername} currentuser={username}/>
            </div>
          </div>
